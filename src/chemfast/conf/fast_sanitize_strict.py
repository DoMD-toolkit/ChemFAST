...
def _format_property_failure(sub_mol: Chem.Mol, sub_to_global_atom: list[int], center_idx: int) -> str:
...
def _build_local_submol(
    mol: Chem.Mol, atom_indices: np.ndarray, bond_indices: np.ndarray, bond_cache: list[Chem.Bond]
...
def _copy_atom_state(source_atom: Chem.Atom, target_atom: Chem.Atom) -> None:
...
def _copy_bond_state(source_bond: Chem.Bond, target_bond: Chem.Bond, sub_to_global_atom: list[int]) -> None:
...
def fast_sanitize(mol: Chem.Mol, max_path: int = 12, buffer_size: int = 3) -> Chem.Mol:
    ...
    The slice radius is max_path + buffer_size. Ring information is
    accepted within max_path, while sanitized atom and bond states are
    committed within max_path // 2.
    """
    logger.warning(f"FAST SANITIZE MODE IS ENABLED! LOCAL RINGS ARE PERCEIVED WITHIN {max_path} BONDS.")
    ...
    for process_hydrogens in (False, True):
        for center_idx in range(num_atoms):
            ...
            # Extract and sanitize the buffered local fragment.
            (
                sub_mol,
                sub_to_global_atom,
                sub_to_global_bond,
            ...
            for sub_ring_atoms, sub_ring_bonds in zip(
                ...
                if any(distance[atom_idx] > max_path for atom_idx in global_ring_atoms):
                    continue
