...
def _is_aromatic_bond(bond: Chem.Bond) -> bool:
    return bond.GetIsAromatic() or bond.GetBondType() == Chem.BondType.AROMATIC
...
def _clear_broken_aromaticity(
    ...
    """Clear aromatic components cut by the fragment boundary."""
    broken_atoms = bytearray(sub_mol.GetNumAtoms())
    stack = []

    Chem.FastFindRings(sub_mol)
    ring_info = sub_mol.GetRingInfo()

    # Any cut bond changes the valence or hydrogen state of an aromatic
    # boundary atom. This also covers aromatic n(C) becoming n after
    # its non-aromatic substituent is removed by the slice.
    for sub_atom_idx, global_atom_idx in enumerate(sub_to_global_atom):
        global_atom = mol.GetAtomWithIdx(global_atom_idx)
        sub_atom = sub_mol.GetAtomWithIdx(sub_atom_idx)

        if global_atom.GetIsAromatic() and sub_atom.GetDegree() < global_atom.GetDegree():
            broken_atoms[sub_atom_idx] = 1
            stack.append(sub_atom_idx)

    # Aromaticity is a property of the complete connected aromatic
    # system, so the whole local component containing a cut bond must
    # be cleared before kekulization.
    while stack:
        sub_atom_idx = stack.pop()

        for bond in sub_mol.GetAtomWithIdx(sub_atom_idx).GetBonds():
            neighbor_idx = bond.GetOtherAtomIdx(sub_atom_idx)
            neighbor = sub_mol.GetAtomWithIdx(neighbor_idx)

            if not neighbor.GetIsAromatic():
                continue

            if not _is_aromatic_bond(bond) and not ring_info.NumBondRings(bond.GetIdx()):
                continue

            if not broken_atoms[neighbor_idx]:
                broken_atoms[neighbor_idx] = 1
                stack.append(neighbor_idx)

    for sub_atom_idx, is_broken in enumerate(broken_atoms):
        if is_broken:
            sub_mol.GetAtomWithIdx(sub_atom_idx).SetIsAromatic(False)

    for bond in sub_mol.GetBonds():
        if not (broken_atoms[bond.GetBeginAtomIdx()] or broken_atoms[bond.GetEndAtomIdx()]):
            continue
...
def fast_sanitize(
    ...
    The slice radius is r_cut + r_buf. Boundary-affected aromatic
    systems expand the local radii by r_buf up to max_r_cut.
    """
    ...
    for process_hydrogens in (False, True):
        for center_idx in range(num_atoms):
            ...
            while effective_r_cut < max_r_cut:
                radius_growth = effective_r_cut - r_cut
                slice_radius = effective_r_cut + r_buf
                trust_radius = effective_r_cut
                commit_radius = base_commit_radius + radius_growth
                stamp += 1
                ...
                broken_atoms = _clear_broken_aromaticity(
                    sub_mol,
                    mol,
                    sub_to_global_atom,
                )

                broken_reaches_commit = any(
                    is_broken and distance[sub_to_global_atom[sub_atom_idx]] <= commit_radius
                    for sub_atom_idx, is_broken in enumerate(broken_atoms)
                )

                if not broken_reaches_commit:
                    ...
                    if not (failed_op == Chem.SanitizeFlags.SANITIZE_KEKULIZE and any(broken_atoms)):
                        break
                ...
                raise ValueError(
                    "Unable to fast sanitize around " f"global atom {center_idx}: " f"max_r_cut={max_r_cut} reached")
            ...
            for sub_ring_atoms, sub_ring_bonds in zip(
                ...
                if any(distance[atom_idx] > trust_radius for atom_idx in global_ring_atoms):
                    continue
